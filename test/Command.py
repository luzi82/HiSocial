...
class TestCommand(_testcommon.HsTest):
    ...
    def test_guest_ping_pass(self):
        ...
        for c in cv :
            r = hs_command.call("base", "guest_ping", {"txt_value":c[0].upper()})
            self.check_ok(r)
            self.assertEqual(len(r), 2)
            self.assertEqual(r["value"],c[1].lower())
            r = hs_command.call("base", "guest_ping", {"txt_value":c[0].lower()})
            self.check_ok(r)
            self.assertEqual(len(r), 2)
            self.assertEqual(r["value"],c[1].lower())
            r = hs_command.call("base", "guest_ping", {"txt_value":c[1].upper()})
            self.check_ok(r)
            self.assertEqual(len(r), 2)
            self.assertEqual(r["value"],c[0].lower())
            r = hs_command.call("base", "guest_ping", {"txt_value":c[1].lower()})
            self.check_ok(r)
            self.assertEqual(len(r), 2)
            self.assertEqual(r["value"],c[0].lower())
    ...
    def test_guest_ping_fail(self):
        ...
        for c in cv :
            r = hs_command.call("base", "guest_ping", {"txt_value":c})
            self.assertTrue(r != None)
            self.assertTrue(isinstance(r,dict))
            self.assertEqual(len(r), 2)
            self.assertEqual(r[hs_command.RESULT_KEY], hs_command.RESULT_VALUE_FAIL_TXT)
    ...
    def test_test_module(self):
        ret = hs_command.call("test","helloworld")
        self.check_ok(ret)
        self.assertEqual(ret["value"], "helloworld")

    def test_arg(self):
        ret = hs_command.call("test","uppercase",{"txt_a":"asdf"})
        self.check_ok(ret)
        self.assertEqual(ret["value"], "ASDF")

    def test_arg_filter(self):
        ret = hs_command.call("test","uppercase_arg",{"txtf_test_upper":"qwer"})
        self.check_ok(ret)
        self.assertEqual(ret["value"], "QWER")

    def test_hellofile(self):
        ret = hs_command.get_file("test","hellofile")
        self.check_ok(ret)
    ...
    def test_hellofile2(self):
        ret = hs_command.get_file("test","hellofile2")
        self.check_ok(ret)
        self.assertEqual(ret["file_type"], "local")
