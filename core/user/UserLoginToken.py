...
def generate_user_login_token(user_id,deadline=None):
    ...
    return Secret.encrypt({"user_id":user_id,"deadline":deadline}, core_config.USER_TOKEN_HASH_HMAC, core_config.USER_TOKEN_ENC_KEY)
...
def check_user_login_token(user_login_token):
    ...
    if(not isinstance(user_login_token, str)):
        return None
    v = Secret.decrypt(user_login_token, core_config.USER_TOKEN_HASH_HMAC, core_config.USER_TOKEN_ENC_KEY)
    if(v == None):
        return None
    now = int(time.time())
    if now > v["deadline"]:
