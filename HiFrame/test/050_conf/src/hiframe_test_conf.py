...
class HiframeTestConf(unittest.TestCase):
    ...
    def test_basic(self):
        hf = hiframe.HiFrame(plugin_path_list=[MY_ABSOLUTE_PARENT],conf_file=RES_FOLDER+"/basic.conf")
    ...
    def test_conf_not_exist(self):
        try:
            hiframe.HiFrame(plugin_path_list=[MY_ABSOLUTE_PARENT],conf_file=RES_FOLDER+"/not_exist.conf")
            # bad
            self.fail()
        except IOError:
            # good
